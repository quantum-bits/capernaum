...
export class PredictionTablesFixture extends AbstractFixture {
  delete() {
    debug("Delete prediction table");
    return PredictionTableModel.query().delete();
  }

  insert() {
    debug("Insert prediction table and entries");
    return PredictionTableModel.query().insertGraph(predictionTable, {
      allowRefs: true,
    });
  }
}
